...
const BASE_TEX = '/textures';
const LEATHER = [
  { img: `${BASE_TEX}/leather-oxblood.png`,  pos: '20% center' },
  { img: `${BASE_TEX}/leather-navy.png`,     pos: '40% center' },
  { img: `${BASE_TEX}/leather-green.png`,    pos: '30% center' },
  { img: `${BASE_TEX}/leather-tobacco.png`,  pos: '50% center' },
  { img: `${BASE_TEX}/leather-oxblood.png`,  pos: '70% center' },
  { img: `${BASE_TEX}/leather-navy.png`,     pos: '60% center' },
  { img: `${BASE_TEX}/leather-green.png`,    pos: '10% center' },
  { img: `${BASE_TEX}/leather-tobacco.png`,  pos: '80% center' },
];
...
function CoordReveal({ coords, visible }: {
  ...
  const items = [
    { label: 'Hexagon', value: coords.hexagon },
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{
        ...
        <div style={{
          fontSize: 11, color: PARCHMENT, fontFamily: 'monospace',
          wordBreak: 'break-all', lineHeight: 1.6,
          userSelect: 'all', cursor: 'text',
          padding: '6px 10px',
          background: 'rgba(0,0,0,0.2)',
          borderRadius: 3,
          border: `1px solid ${GOLD_DIM}22`,
        }}>{hexItem.value}</div>
...
export default function Babel() {
  ...
  const fetchPage = useCallback(async (hex: string, w: number, s: number, v: number, text = '', pg = 1): Promise<PageData> => {
    ...
    const res = await fetch(`${BASE_PATH}/api/babel?${params.toString()}`);
  ...
  async function findAddress() {
    ...
    try {
      const res = await fetch(`${BASE_PATH}/api/babel`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: inputText }) });
          ...
          {mode === 'find' && (
            <div>
              ...
              <div style={{ marginBottom: 22 }}>
                <div style={{ fontSize: 9, color: FADED, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 10 }}>
                  Known texts — click to locate
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {[
                    { label: 'Call me Ishmael.', text: 'Call me Ishmael.' },
                    { label: 'It was the best of times', text: 'It was the best of times, it was the worst of times.' },
                    { label: 'In the beginning', text: 'In the beginning God created the heavens and the earth.' },
                    { label: 'To be, or not to be', text: 'To be, or not to be, that is the question.' },
                    { label: 'Your name', text: '' },
                  ].map(({ label, text }) => (
                    <button
                      key={label}
              ...
              {activePage && !pageLoading && (
                <div style={{ marginTop: 20 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginTop: 14 }}>
                    <button
                      disabled={currentPageNum <= 1 || pageLoading}
                      onClick={async () => {
                        const pg = currentPageNum - 1;
                        setCurrentPageNum(pg);
                        setPageLoading(true);
                        const addrKey = `${activePage.hex}-${activePage.wall}-${activePage.shelf}-${activePage.volume}`;
                        const searchText = addressBook.current[addrKey] ?? exploreSearchText;
                        const pd = await fetchPage(activePage.hex, activePage.wall, activePage.shelf, activePage.volume, searchText, pg);
                        setActivePage(pd); setPageLoading(false);
                      }}
                      style={{ background: 'none', border: `1px solid ${GOLD_DIM}44`, color: `${FADED}cc`, borderRadius: 3, padding: '4px 14px', cursor: 'pointer', fontFamily: '"Palatino Linotype",Palatino,Georgia,serif', fontStyle: 'italic', fontSize: 12 }}
                    >← Prev</button>
                    <span style={{ color: FADED, fontStyle: 'italic', fontSize: 12, fontFamily: '"Palatino Linotype",Palatino,Georgia,serif' }}>
                      Page {currentPageNum} of 410
                    </span>
                    <button
                      disabled={currentPageNum >= 410 || pageLoading}
                      onClick={async () => {
                        const pg = currentPageNum + 1;
                        setCurrentPageNum(pg);
                        setPageLoading(true);
